...
run_single_replication <- function(rep_num) {
  ...
  b2_func_sim <- function(x_vals, z_val) {
    x1 <- x_vals[1]; x2 <- x_vals[2]
    x3 <- x_vals[3]; x4 <- x_vals[4]; x5 <- x_vals[5]
  ...
  for (i in 1:n) {
    b2_true_log[i] <- b2_func_sim(X[i,], Z[i])
  }

  sigma2_true <- 1.5
  logT_true <- b2_true_log + W_true[cluster_id] + rnorm(n, 0, sqrt(sigma2_true))
  ...
  cat("Computing true CRATE for 4 counties at 5 time points...\n")
  ...
  for (row in 1:nrow(true_values)) {
    ...
    for (j in 1:n_i) {
      x_j <- X_i[j, ]
      b2_1_i[j] <- b2_func_sim(x_j, 1)
      b2_0_i[j] <- b2_func_sim(x_j, 0)
    }

    b2_1_i_full <- b2_1_i + W_i_true
    b2_0_i_full <- b2_0_i + W_i_true

    sigma_m <- sqrt(sigma2_true)
    CRATE_i_j <- numeric(n_i)

    for (j in 1:n_i) {
      z1_j <- (log(t_star) - b2_1_i_full[j]) / sigma_m
      z0_j <- (log(t_star) - b2_0_i_full[j]) / sigma_m

      mu1_j <- exp(b2_1_i_full[j] + sigma2_true / 2)
      mu0_j <- exp(b2_0_i_full[j] + sigma2_true / 2)
...
write.csv(all_results_df, "Sim1_combined_simulation_detailed_results.csv", row.names = FALSE)
write.csv(summary_clean, "Sim1_combined_simulation_summary.csv", row.names = FALSE)

cat("\nDetailed results saved to: Sim1_combined_simulation_detailed_results.csv\n")
cat("Summary results saved to: Sim1_combined_simulation_summary.csv\n")

cat("\n========================================\n")
cat("COMPARISON OF THREE METHODS FOR 4 COUNTIES AT 5 TIME POINTS\n")
cat("========================================\n")
...
save(all_results, file = "Sim1_three_methods_sim_all_results_list.RData")
cat("\nAll results list saved to: Sim1_three_methods_sim_all_results_list.RData\n")
