...
\usepackage[orientation=landscape,size=a0,scale=1.02]{beamerposter}
\usepackage{graphicx}
\usepackage{booktabs}
...
\begin{keybox}
\centering
\textbf{Does delaying breast cancer treatment ($>$90 days) \emph{cause} worse survival? How does the effect vary across geographic regions?}
\end{keybox}
...
\item Confounders: Age, HR, Tumor Grade, Biopsy Delay
\end{itemize}
\end{minipage}
\end{posterblock}
...
\begin{posterblock}{The Gap --- No Unified Method Exists}
\centering
\begin{adjustbox}{width=0.92\textwidth}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{l c c c c}
\toprule
\textbf{Method} & \textbf{Spatial} & \textbf{Nonparametric} & \textbf{Survival} & \textbf{Causal} \\
\midrule
IPW / PSW & $\times$ & $\times$ & $\times$ & $\checkmark$ \\
riAFTBART & $\times$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
Spatial Frailty & $\checkmark$ & $\times$ & $\checkmark$ & $\times$ \\
\rowcolor{vlgreen} \textbf{PS-LND (Ours)} & $\boldsymbol{\checkmark}$ & $\boldsymbol{\checkmark}$ & $\boldsymbol{\checkmark}$ & $\boldsymbol{\checkmark}$ \\
\bottomrule
\end{tabular}
\end{adjustbox}
\raggedright
\vspace{3pt}
\begin{itemize}
\item \textbf{PS-LND} is the first to unify all four in a single Bayesian framework
\end{itemize}
\end{posterblock}

\vspace{4pt}

\begin{posterblock}{Causal Estimands}
...
\begin{itemize}
\item \textbf{County:}\; $\text{ACERM}_i(t^*) = \frac{1}{n_i}\sum_{j}\text{CERM}_{ij}(t^*)$\; (also ACERMT, ACERMU)
\item \textbf{State:}\; $\text{RATE}(t^*) = \frac{1}{N}\sum_{i,j}\text{CERM}_{ij}(t^*)$\; (RATT, RATU)
\item \textbf{Survival-prob:}\; $\text{SPTE}_i(t) = S^{(1)}_i(t) - S^{(0)}_i(t)$
\end{itemize}

\vspace{3pt}
{\color{FSUgarnet}\textbf{Identifiability:}} SUTVA, Consistency, Unconfoundedness, Positivity, Non-informative censoring

\vspace{5pt}
\centering
\includegraphics[width=\textwidth]{BD_HR.png}
\end{posterblock}
...
\vspace{-7pt}
\begin{itemize}
\item Separate prognostic $\mu$ \& treatment $\tau$ surfaces handle \textbf{targeted selection}; $\hat{e}_{ij}$ enters as confounder
...
\begin{minipage}[t]{0.49\textwidth}
\begin{itemize}[itemsep=4pt]
\item[\color{accentgreen}\Large$\checkmark$] \textbf{No feedback:} two-stage design isolates PS from misspecified outcome
...
$K\!=\!35$ clusters on a $5\!\times\!7$ grid, $N\!=\!1750$, 5 confounders, 150 replications.
...
\midrule
\textbf{1.\ Ideal} & Correct specification & Log-Normal $+$ DAGAR ($\rho_w\!=\!0.7$); nonlinear main effects $+$ treatment interactions \\[2pt]
\textbf{2.\ Misspec.} & Robustness to model error & Adds covariate $\times$ random-effect interactions $g(\mathbf{x},z,w)$ omitted by fitted model \\[2pt]
\textbf{3.\ Stress} & Distribution violation $+$ heavy censoring & Weibull (not log-Normal), 50\% censoring, strong confounding \\
\bottomrule
\end{tabularx}
...
\begin{appblock}{Application: Florida Cancer Registry --- WA-Distant}

{\color{accentgreen!80!black}\textbf{Patient-Level Effects (Broward County)}}\\[2pt]
\centering
\includegraphics[width=\textwidth]{poster_figs/cerm_plot.png}\\
\raggedright
...
\item With biopsy delay: effect vanishes (CI $\ni$ 0)
\end{itemize}

\vspace{5pt}
{\color{accentgreen!80!black}\textbf{County-Level Spatial Patterns}}\\[2pt]
\centering
\includegraphics[width=\textwidth]{poster_figs/crate-1.png}\\
\raggedright
...
\vspace{3pt}
\begin{itemize}
\item All CIs exclude zero $\Rightarrow$ strong evidence of benefit
\item RATU $>$ RATT: long-TD patients gain the most
\end{itemize}

\vspace{3pt}
\begin{findingbox}
{\color{accentgreen!80!black}\textbf{Survival Probability:}} Peak \textbf{11\%} survival-probability gain at $t\!=\!2.2$ yr; CI $> 0$ for all $t$\\[2pt]
{\color{accentgreen!80!black}\textbf{Spatial Fit:}} $\hat{\rho}_w = 0.72$ (strong spatial borrowing); $\hat{\rho}_v = 0.58$ (spatial confounding present)
\end{findingbox}
...
\item Spatial model reveals \textbf{county-level heterogeneity} missed by global estimates
\end{itemize}
\end{keybox}
